...
const CartModal: React.FC<CartModalProps> = ({ isOpen, onClose }) => {
  ...
  const generateWhatsAppMessage = () => {
    ...
    }\n\nCEP: ${cep}\n Frete será calculado via WhatsApp\n\nTotal: ${formatFullPrice(total)}${customerInfo}`;
  ...
  const handleWhatsAppClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    // Prosseguir com WhatsApp
    window.open(generateWhatsAppMessage(), '_blank');
  };

  const handleAuthSuccess = () => {
    setShowAuthModal(false);
    toast.success('Agora você pode finalizar sua compra!');
  };
          ...
          {items.length > 0 && (
            <div className="border-t dark:border-gray-700 bg-white dark:bg-gray-800 flex-shrink-0">
              ...
              <div className="p-4 pt-0">
                ...
                >
                  {isAuthenticated ? 'Finalizar no WhatsApp' : 'Fazer Login e Finalizar'}
                </a>
                {!isAuthenticated && (
